...
2. A **web server** (`src/main.rs`) built with the Axum framework that handles OAuth 2.0 token management (refreshing
   tokens and checking their status).

The project uses `reqwest` for making HTTP requests, `serde` for JSON serialization/deserialization, and `hmac` for
signing API requests. Configuration is managed through a `.env` file.
...
- **Linting:** The project uses `clippy` for linting. Please run `cargo clippy` to check for common mistakes and style
  issues.
...
- **Known Issue:** The `README.md` mentions a known issue with API signature validation. Please refer to the `README.md`
  for more details.
